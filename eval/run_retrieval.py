...
def main() -> None:
    data = json.loads((EVAL_DIR / "retrieval_queries.json").read_text())
    ...
    for q in queries:
        ...
        rows["rerank"].append({**s_rr, "category": q["category"]})
        rows["bienc"].append({**s_bi, "category": q["category"]})
        per_query.append({
    ...
    print(header)
    print("-" * len(header))
    summary = {"n_queries": len(queries), "k": K, "rerank": {}, "bienc": {}, "delta": {}}
    ...
    # Per-category recall@10 (reranked).
