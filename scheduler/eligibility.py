...
def load_graph(program: str = "BSCS-DM", term: str = "202601") -> nx.DiGraph:
    path = DATA_DIR / f"{program}_{term}_graph.gpickle"
    if not path.exists():
        raise SystemExit(f"Missing {path} -- run scraper/build_graph.py first.")
